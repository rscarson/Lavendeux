	...
	function renderExtensionRow(extension) {
		return (
			<tr key={extension.filename}>
				...
				<td>
					{Object.keys(extension.functions).length}
					{' '}
					{lang.extensionview_functions}
					,
					{' '}
					{Object.keys(extension.decorators).length}
					{' '}
					{lang.extensionview_decorators}
				</td>
				<td>{extension.author}</td>
				<td>
					<IconButton
						variant="secondary"
						onClick={() => run('disable_extension', { srcPath: extension.filename })}
						icon="x-circle"
						title={lang.extensionview_btn_disable}
					/>
&nbsp;
				</td>
			</tr>
...
&nbsp;
					<IconButton variant="secondary" onClick={reloadExtensions} icon="arrow-clockwise" title={lang.extensionview_btn_reload} />
&nbsp;
